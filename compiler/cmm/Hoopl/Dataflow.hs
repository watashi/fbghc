...
fixpointAnal direction DataflowLattice{ fact_bot = _, fact_join = join }
  ...
  where
    ...
    start      = {-# SCC "start" #-} IntSet.fromDistinctAscList [0..n-1]
    ...
    loop t fbase | Just (ix, todo) <- IntSet.minView t =
           let
               blk = block_arr ! ix
...
fixpoint direction DataflowLattice{ fact_bot = _, fact_join = join }
  ...
  where
    ...
    loop t fbase !newblocks | Just (ix, todo) <- IntSet.minView t = do
    ...
    loop _ fbase newblocks = return (fbase, newblocks)
...
mkDepBlocks :: NonLocal n => Direction -> [Block n C C] -> LabelMap IntSet
mkDepBlocks Fwd blocks = go blocks 0 mapEmpty
  where go []     !_  m = m
        go (b:bs) !n m = go bs (n+1) $! mapInsert (entryLabel b) (IntSet.singleton n) m
mkDepBlocks Bwd blocks = go blocks 0 mapEmpty
  where go []     !_ m = m
        go (b:bs) !n m = go bs (n+1) $! go' (successors b) m
            where go' [] m = m
                  go' (l:ls) m = go' ls (mapInsertWith (IntSet.union) l (IntSet.singleton n) m)
...
--
updateFact :: JoinFun f -> LabelMap IntSet
           -> Label -> f       -- out fact
           -> (IntHeap, FactBase f)
           -> (IntHeap, FactBase f)
