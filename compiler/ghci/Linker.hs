...
unload_wkr hsc_env keep_linkables pls = do
  let (objs_to_keep, bcos_to_keep) = partition isObjectLinkable keep_linkables
      ...
      (objs_to_unload, remaining_objs_loaded) =
         partition (discard objs_to_keep_env) (objs_loaded pls)
      (bcos_to_unload, remaining_bcos_loaded) =
         partition (discard bcos_to_keep_env) (bcos_loaded pls)
  ...
  itbl_env'     <- evaluate $ force $ filterNameEnv keep_name (itbl_env pls)
  closure_env'  <- evaluate $ force $ filterNameEnv keep_name (closure_env pls)
