...
module RtClosureInspect(
     ...
     GenClosure(..), ClosureType(..), isConstr, isIndirection
 ) where
...
constrClosToName hsc_env
  GenClosure{tipe=Constr, constrName = Just (pkg,modl,occ)} = do
   let occName = mkOccName OccName.dataName occ
       modName = mkModule (stringToUnitId pkg) (mkModuleName modl)
   Right `fmap` lookupOrigIO hsc_env modName occName
constrClosToName _hsc_env clos =
   return (Left ("conClosToName: Expected ConstrClosure, got " ++
      show (fmap (const ()) clos)))
...
cPprTermBase y =
 ...
 where
   ...
   -- size here.
   ppr_double  _ Term{subTerms=[Prim{value=[w1,w2]}]} = do
      let f = unsafeDupablePerformIO $
                alloca $ \p -> do
   ...
   --
   ppr_integer _ Term{subTerms=[Prim{value=[W# w]}]} =
      return (Just (Ppr.integer (S# (word2Int# w))))
   ppr_integer _ Term{dc=Right con,
                      subTerms=[Term{subTerms=[Prim{value=ws}]}]} = do
   ...
   ppr_list p (Term{subTerms=[h,t]}) = do
       let elems      = h : getListTerms t
           ...
           chars = [ chr (fromIntegral w)
                   | Term{subTerms=[Prim{value=[w]}]} <- elems ]
...
-- Thunks we may want to force
      t | isThunk t && force -> do
         traceTR (text "Forcing a " <> text (show (fmap (const ()) clos)))
...
-- We also follow references
      MutVar _
         ...
         x <- go (pred max_depth) contents_tv contents_ty (ptrs clos ! 0)
 ...
 -- The interesting case
      Constr -> do
        ...
        Right dcname <- trIO $ constrClosToName hsc_env clos
      ...
      -- bytes of the array.
      ArrWords -> do
         let bytes:ws = nonPtrs clos
         traceTR (text "ByteArray# closure, size " <> ppr bytes)
